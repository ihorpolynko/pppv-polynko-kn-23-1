...
namespace QuizApp.Data
{
    public class SqlQuizRepository : IQuizRepository
    {
        ...
        public Quiz GetById(int id)
        {
            ...
            {
                conn.Open();
                using (var cmd = new SqlCommand("SELECT QuizId, Title, Category FROM Quizzes WHERE QuizId = @id", conn))
                {
                    ...
                    using (var r = cmd.ExecuteReader())
                    {
                        if (r.Read())
                        {
                            ...
                            quiz.Category = (string)r["Category"];
                        }
                        else return null;
                    }
                }
        ...
        public void Add(Quiz quiz)
        {
            using (var conn = new SqlConnection(_conn))
            {
                ...
                using (var cmd = new SqlCommand(
                    "INSERT INTO Quizzes (Title, Category) VALUES (@title, @category); SELECT CAST(SCOPE_IDENTITY() AS INT);", conn))
                {
                    cmd.Parameters.AddWithValue("@title", quiz.Title ?? "");
                    cmd.Parameters.AddWithValue("@category", quiz.Category ?? "");
        ...
        public void Update(Quiz quiz)
        {
            using (var conn = new SqlConnection(_conn))
            {
                conn.Open();
                using (var cmd = new SqlCommand(
                    "UPDATE Quizzes SET Title=@title, Category=@category WHERE QuizId=@id", conn))
                {
                    cmd.Parameters.AddWithValue("@title", quiz.Title ?? "");
                    cmd.Parameters.AddWithValue("@category", quiz.Category ?? "");
