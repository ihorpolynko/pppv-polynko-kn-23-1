...
namespace ClanBattle.Services
{
    public class CombatService
    {
        ...
        private void Attack(IUnit attacker, IUnit defender)
        {
            ...
            double extraDefense = 1.0;
            double extraDodge = 1.0;

            if (attacker is UnitDecoratorBase aDec) extraDamage *= aDec.DamageModifier();
            if (defender is UnitDecoratorBase dDec)
            {
                extraDefense *= dDec.DefenseModifier();
                extraDodge *= dDec.DodgeModifier();
            }

            baseDamage = (int)(baseDamage * extraDamage);
            positionDefenseBonus *= extraDefense;

            if (_rnd.NextDouble() < dodgeChance * extraDodge)
            {
            ...
            Console.WriteLine($"{attacker.Name} атакує {defender.Name} ({totalDamage} dmg) | HP {defender.Health}");
        }
    }
}
